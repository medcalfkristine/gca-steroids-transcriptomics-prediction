...
# Prepare volcano_df with significance
...
# Select top upregulated and downregulated genes by absolute log2FC
...
# Define group colors (change names and colors as needed)
group_colors <- list(
  group = c(
...
```{r}
# Create a data frame with key info
volcano_df <- res_df
...
ggplot(volcano_df, aes(x = log2FoldChange, y = log10padj)) +
  ...
  theme_classic() +
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5, size = rel(1.2), color = "black")  # <- updated
  ) +
...
top_genes <- volcano_df[volcano_df$significance == "Significant", ]
top_genes <- top_genes[order(abs(top_genes$log2FoldChange), decreasing = TRUE), ]
top_genes <- head(top_genes, 10)  # label top 10
...
#### <3 vs 3–7
```{r, echo = FALSE, message=FALSE, warning=FALSE}
lt3 <- rbind(protein_coding_less3, protein_coding_days_3_4)
lt3_up <- lt3 %>% filter(log2FoldChange > 1 & !is.na(hgnc_symbol) & hgnc_symbol != "")
lt3_ids <- bitr(lt3_up$hgnc_symbol, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Hs.eg.db)

three7_up <- protein_coding_days_3_7 %>% filter(log2FoldChange > 1 & !is.na(hgnc_symbol) & hgnc_symbol != "")
three7_ids <- bitr(three7_up$hgnc_symbol, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Hs.eg.db)

gene_list <- list("lt3" = unique(lt3_ids$ENTREZID), "3_7" = unique(three7_ids$ENTREZID))
compare <- compareCluster(geneCluster = gene_list, fun = "enrichGO", OrgDb = org.Hs.eg.db, ont = "BP", keyType = "ENTREZID", pvalueCutoff = 0.05)
dotplot(compare, showCategory = 8, title = "GO Compare: <3 vs 3–7")
```
...
gt7_up <- protein_coding_more_7 %>% filter(log2FoldChange > 1 & !is.na(hgnc_symbol) & hgnc_symbol != "")
gt7_ids <- bitr(gt7_up$hgnc_symbol, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Hs.eg.db)

gene_list <- list("3_7" = unique(three7_ids$ENTREZID), "gt7" = unique(gt7_ids$ENTREZID))
compare <- compareCluster(geneCluster = gene_list, fun = "enrichGO", OrgDb = org.Hs.eg.db, ont = "BP", keyType = "ENTREZID", pvalueCutoff = 0.05)
dotplot(compare, showCategory = 8, title = "GO Compare: 3–7 vs >7")
```

```{r}
# Significant DEGs (padj < 0.05)
deg_less3   <- rownames(res_less3[which(res_less3$padj < 0.05), ])
deg_3_7     <- rownames(res_days_3_7[which(res_days_3_7$padj < 0.05), ])
deg_more7   <- rownames(res_more_7[which(res_more_7$padj < 0.05), ])

# Counts of unique vs shared
all_degs <- list("<3d" = deg_less3, "3-7d" = deg_3_7, ">7d" = deg_more7)
...
# Numbers
lengths(all_degs)                         # total per group
length(Reduce(intersect, all_degs))       # shared across all groups

# Extract top 50 by |log2FC|
topN <- 50
top_less3 <- rownames(head(res_less3[order(abs(res_less3$log2FoldChange), decreasing = TRUE), ], topN))
top_3_7   <- rownames(head(res_days_3_7[order(abs(res_days_3_7$log2FoldChange), decreasing = TRUE), ], topN))
top_more7 <- rownames(head(res_more_7[order(abs(res_more_7$log2FoldChange), decreasing = TRUE), ], topN))

# Overlap
Reduce(intersect, list(top_less3, top_3_7, top_more7))
length(Reduce(intersect, list(top_less3, top_3_7, top_more7))) # how many overlap

# Create data frames of log2FC for shared genes
fc_df <- data.frame(
  less3 = res_less3$log2FoldChange,
  day3_7 = res_days_3_7$log2FoldChange,
  more7 = res_more_7$log2FoldChange
)
rownames(fc_df) <- rownames(res_less3)

# Keep only complete cases (no NA)
fc_df <- na.omit(fc_df)

# Correlation matrix
cor_mat <- cor(fc_df, method = "pearson")
cor_mat

# Example: correlation less3 vs 3-7
cor(fc_df$less3, fc_df$day3_7, use = "complete.obs")
...
less3_ranks <- res_df_less3 %>%
  filter(!is.na(log2FoldChange), !is.na(hgnc_symbol), hgnc_symbol != "") %>%
  group_by(hgnc_symbol) %>% 
  summarise(score = mean(log2FoldChange)) %>%
  arrange(desc(score))

# Convert to ENTREZ IDs
entrez_ids <- bitr(less3_ranks$hgnc_symbol, fromType = "SYMBOL",
                   toType = "ENTREZID", OrgDb = org.Hs.eg.db)

# Join ranks with ENTREZ IDs
less3_ranks <- left_join(entrez_ids, less3_ranks, by = c("SYMBOL" = "hgnc_symbol")) %>%
  distinct(ENTREZID, .keep_all = TRUE) %>%
  drop_na()

geneList_less3 <- less3_ranks$score
names(geneList_less3) <- less3_ranks$ENTREZID
geneList_less3 <- sort(geneList_less3, decreasing = TRUE)

# Run GSEA
gsea_less3 <- gseGO(
  geneList = geneList_less3,
  OrgDb = org.Hs.eg.db,
  ont = "BP",
  keyType = "ENTREZID",
  minGSSize = 10,
...
dotplot(gsea_less3, showCategory = 9, title = "GSEA – <3 Days (BP)")
...
```{r}
# 1. Prepare ranked gene list (log2FoldChange named by SYMBOL)
gsea_input <- res_df_days_3_7 %>%
  dplyr::filter(!is.na(log2FoldChange), !is.na(hgnc_symbol), hgnc_symbol != "") %>%
  dplyr::distinct(hgnc_symbol, .keep_all = TRUE) %>%
  dplyr::arrange(desc(log2FoldChange)) %>%
  dplyr::pull(log2FoldChange)

names(gsea_input) <- res_df_days_3_7 %>%
  dplyr::filter(!is.na(log2FoldChange), !is.na(hgnc_symbol), hgnc_symbol != "") %>%
  dplyr::distinct(hgnc_symbol, .keep_all = TRUE) %>%
  dplyr::pull(hgnc_symbol)

gsea_input <- sort(gsea_input, decreasing = TRUE)  # Must be sorted

# 2. Run GSEA for GO Biological Process (BP)
gsea_result <- gseGO(
  geneList = gsea_input,
  OrgDb = org.Hs.eg.db,
  keyType = "SYMBOL",
...
# 3. Plot results
dotplot(gsea_result, showCategory = 10, title = "GSEA – GO Biological Process")
...
entrez_ids <- bitr(gt7_ranks$hgnc_symbol, fromType = "SYMBOL",
                   toType = "ENTREZID", OrgDb = org.Hs.eg.db)

# Merge and clean gene list
gt7_ranks <- left_join(entrez_ids, gt7_ranks, by = c("SYMBOL" = "hgnc_symbol")) %>%
  distinct(ENTREZID, .keep_all = TRUE) %>%
  drop_na()

# Format for GSEA
geneList_gt7 <- gt7_ranks$score
names(geneList_gt7) <- gt7_ranks$ENTREZID
geneList_gt7 <- sort(geneList_gt7, decreasing = TRUE)

# Run GSEA
gsea_gt7 <- gseGO(
  geneList = geneList_gt7,
  OrgDb = org.Hs.eg.db,
  ont = "BP",
  keyType = "ENTREZID",
  minGSSize = 10,
...
dotplot(gsea_gt7, showCategory = 9, title = "GSEA – >7 Days (BP)")
...
```{r}
# Initialize list to hold GO results
...
```{r}
# Extract all GO term descriptions for each enrichment result
go_top_terms <- lapply(go_results_list, function(df) {
  df <- df[order(df$p.adjust), ]   # sort by adjusted p-value
  df$Description                  # keep all descriptions
})

# Determine max length (to pad shorter sets)
max_len <- max(sapply(go_top_terms, length))

# Pad each list with NA so all columns align
...
| Tier | Genes to analyze                | Purpose                    |
| ---- | ------------------------------- | -------------------------- |
| 1    | Top 30 L2FC                     | Unbiased strongest signals |
| 2    | Known GCA-related genes         | Biological relevance       | 
| 3    | Steroid-response genes          | Mechanism of treatment     |
| 4    | Genes in enriched GO/KEGG terms | Connect stats to biology   |

*need to look at 2-3 also within the Venn diagram section
